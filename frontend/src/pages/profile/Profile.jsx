     ...
     const fetchFriendRequests = async () => {
            try {
                ...
                const requestProfiles = await Promise.all(
                    uniqueRequests.filter(request => request.is_active === true).map(async (request) => {
        ...
        <div className="min-h-screen bg-gradient-to-r from-violet-500 to-indigo-600 text-white">
            ...
            <div className="flex flex-col md:flex-row w-full mx-auto gap-6 p-6">
                ...
                {searchResults.length > 0 && (
                    <div ref={searchResultsRef} className="absolute top-16 right-8 w-1/3 md:w-1/4 bg-white rounded-lg shadow-lg p-6">
                        <ul>{searchResults.map((user) => (
                            <li key={user.user_id} className="flex items-center gap-4 border-b border-gray-200 pb-4">
                                <img src={user.profile_image} alt={user.username} className="h-24 w-20 rounded-lg shadow-md"/>
                                <div>
                                    <a className="text-lg font-semibold text-indigo-600 hover:underline" href={`/profile/${user.user_id}`}>
                                        {user.username}
                                    </a>
                                </div>
                                <button className="p-2 bg-green-500 rounded"  ref={searchResultsRef} onClick={() => AddFriend(user.user_id)}>Add</button>
                                <button className="p-2 bg-violet-700 rounded" ref={searchResultsRef} onClick={() => Follow(user.user_id)}>Follow</button>
                            </li>
                        ))}</ul>
                    </div>
                )}
                ...
                <div className="bg-white rounded-lg shadow-lg p-6 md:w-1/2">
                    <p className="text-center font-bold text-xl mb-4 text-gray-800">Following</p>
                    <ul>
                        {profileData.followingProfiles.map(follow => (
                            <li key={follow.id} className="flex items-center gap-4 border-b border-gray-200 pb-4">
                                ...
                                <button className="p-2 bg-red-500 rounded" onClick={() => Unfollow(follow.user_id)}>Unfollow</button>
                            </li>
                        ))}
                    </ul>
                </div>
                ...
                <div className="bg-white rounded-lg shadow-lg p-6 md:w-1/2">
                    <p className="text-black text-center font-semibold">Friend Requests</p>
                    <ul>
                        {profileData.requestProfiles.map(reqProfile => (
                            <li key={reqProfile.id} className="flex items-center gap-4 border-b border-gray-200 pb-4">
                                ...
                                <button
                                    ...
                                    onClick={() => DenyFriend(reqProfile.request_id)}
                                >Deny</button>
                            </li>
                        ))}
                    </ul>
